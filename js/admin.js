open() {
    ...
    $("#admin-body").innerHTML = `
      ...
      <div class="card"><div class="table-wrap"><table class="data">
        <thead><tr>
          ...
          <th class="num">Bestand</th><th>Status</th><th></th>
        </tr></thead>
        <tbody>${
          this.products.length
            ...
            <td class="num">${p.track_stock === false ? '<span class="muted">—</span>' : Number(p.stock ?? 0)}</td>
    ...
    openModal({
      ...
      bodyHTML: `
        ...
        <div class="field">
          ...
          <select class="select" id="f-track">
            <option value="1" ${p?.track_stock !== false ? "selected" : ""}>Ja — Bestand wird beim Verkauf abgezogen</option>
            <option value="0" ${p?.track_stock === false ? "selected" : ""}>Nein — unbegrenzt verfügbar</option>
          </select>
        </div>
        <div class="field">
          <label for="f-stock">Bestand (Stück)</label>
          <input class="input" id="f-stock" type="number" step="1" min="0"
                 value="${p ? Number(p.stock ?? 0) : 0}">
        </div>
        <div class="field">
          <label for="f-min">Mindestbestand — darunter kommt eine Warnung</label>
          <input class="input" id="f-min" type="number" step="1" min="0"
      ...
      onMount: (root) => {
        $("#f-save", root).addEventListener("click", async () => {
          ...
          if (!payload.name)
            return toast("Bitte einen Namen eingeben", "error");
          try {
    ...
    openModal({
      ...
      onMount: (root) => {
        $("#d-save", root).addEventListener("click", async () => {
          ...
          if (!payload.name)
            return toast("Bitte eine Bezeichnung eingeben", "error");
          try {
  ...
  /* ---------- Kooperationen (Rabatt nur mit Codewort) ---------- */

  renderCoops() {
    $("#admin-body").innerHTML = `
      <div class="row" style="margin-bottom:var(--space-4)">
        <button class="btn btn-primary" data-new-coop>+ Neue Kooperation</button>
        <span class="muted" style="font-size:var(--text-sm)">
          Kooperationen sind Rabatte, die an der Kasse erst nach Eingabe des Codeworts gelten.</span>
      </div>
      <div class="card"><div class="table-wrap"><table class="data">
        <thead><tr><th>Partner</th><th>Art</th><th class="num">Rabatt</th>
          <th>Codewort</th><th>Status</th><th></th></tr></thead>
        <tbody>${
          this.coops.length
            ? this.coops
                .map(
            ...
            <td class="muted">${c.kind === "percent" ? "Prozent" : "Fester Betrag"}</td>
            <td class="num">${c.kind === "percent" ? Number(c.value) + " %" : money(c.value)}</td>
            <td><code class="code">${esc(c.code)}</code></td>
            <td><span class="tag ${c.is_active ? "ok" : ""}">${c.is_active ? "Aktiv" : "Inaktiv"}</span></td>
            <td><div class="row" style="gap:var(--space-2);flex-wrap:nowrap">
              <button class="btn btn-sm" data-edit-coop="${c.id}">Bearbeiten</button>
              <button class="btn btn-sm btn-danger" data-del-coop="${c.id}">Löschen</button>
            </div></td></tr>`,
                )
                .join("")
            : `<tr><td colspan="6" class="muted" style="padding:var(--space-8);text-align:center">
                 Noch keine Kooperationen angelegt.</td></tr>`
        }</tbody></table></div></div>`;
  },

  coopForm(c = null) {
    openModal({
      title: c ? "Kooperation bearbeiten" : "Neue Kooperation",
      bodyHTML: `
        <div class="field">
          <label for="c-name">Name des Partners</label>
          <input class="input" id="c-name" value="${esc(c?.name || "")}"
                 placeholder="z. B. Fitnessstudio Nachbarschaft">
        </div>
        <div class="field">
          <label for="c-kind">Art</label>
          <select class="select" id="c-kind">
            <option value="percent" ${c?.kind !== "fixed" ? "selected" : ""}>Prozent vom Gesamtbetrag</option>
            <option value="fixed" ${c?.kind === "fixed" ? "selected" : ""}>Fester Betrag in Euro</option>
          </select>
        </div>
        <div class="field">
          <label for="c-value">Rabatt</label>
          <input class="input" id="c-value" type="number" step="0.5" min="0"
                 inputmode="decimal" value="${c ? Number(c.value) : 15}">
        </div>
        <div class="field">
          <label for="c-code">Codewort</label>
          <input class="input" id="c-code" value="${esc(c?.code || "")}"
                 autocomplete="off" placeholder="z. B. FIT15">
          <p class="muted" style="font-size:var(--text-xs);margin-top:var(--space-2)">
            Nur wer dieses Wort an der Kasse eingibt, bekommt den Rabatt.
            Groß- und Kleinschreibung ist egal.</p>
        </div>
        <div class="field">
          <label for="c-active">Status</label>
          <select class="select" id="c-active">
            <option value="1" ${c?.is_active !== false ? "selected" : ""}>Aktiv</option>
            <option value="0" ${c?.is_active === false ? "selected" : ""}>Inaktiv</option>
          </select>
        </div>`,
      footHTML: `<button class="btn" data-close>Abbrechen</button>
      ...
      onMount: (root) => {
        $("#c-save", root).addEventListener("click", async () => {
          const payload = {
            ...
            is_active: $("#c-active", root).value === "1",
          };
          if (!payload.name)
            return toast("Bitte einen Namen eingeben", "error");
          if (payload.code.length < 3)
            return toast("Das Codewort braucht mindestens 3 Zeichen", "error");
          try {
            if (c) await DB.updateCoop(c.id, payload);
            else await DB.createCoop(payload);
            closeModal();
            toast("Gespeichert");
            await this.loadTab();
          } catch (err) {
            if (String(err.message).includes("cooperations_code_idx"))
              return fail(new Error("Dieses Codewort wird schon verwendet"));
            fail(err);
          }
        });
      },
  ...
  renderStock() {
    const tracked = this.products.filter((p) => p.track_stock !== false);
    const low = tracked.filter(
      (p) => Number(p.stock ?? 0) <= Number(p.min_stock ?? 0),
    );
    const out = tracked.filter((p) => Number(p.stock ?? 0) <= 0);
    const wert = num(
      tracked.reduce(
    ...
    $("#admin-body").innerHTML = `
      <div class="stack">
        <div class="stats" style="margin-bottom:0">
          <div class="stat accent"><div class="stat-label">Warenwert im Lager</div>
            <div class="stat-value">${money(wert)}</div></div>
          <div class="stat"><div class="stat-label">Artikel mit Lager</div>
            <div class="stat-value">${tracked.length}</div></div>
          <div class="stat"><div class="stat-label">Nachbestellen</div>
            <div class="stat-value">${low.length}</div></div>
          <div class="stat"><div class="stat-label">Ausverkauft</div>
            <div class="stat-value">${out.length}</div></div>
        </div>
        ...
        ${
          low.length
            ? `<div class="notice">
                 ...
                 ${low.map((p) => `${esc(p.name)} (${Number(p.stock ?? 0)})`).join(" · ")}
               </div>`
            : ""
        }
        ...
        <div>
          ...
          <div class="card"><div class="table-wrap"><table class="data">
            <thead><tr><th>Artikel</th><th>Kategorie</th><th class="num">Bestand</th>
              <th class="num">Minimum</th><th>Lage</th><th></th></tr></thead>
            <tbody>${
              this.products.length
                ? this.products
                    .map((p) => {
                      const t = p.track_stock !== false;
                      const s = Number(p.stock ?? 0);
                      const m = Number(p.min_stock ?? 0);
                      let lage = '<span class="tag">Ohne Lager</span>';
                      if (t && s <= 0)
                        lage = '<span class="tag bad">Ausverkauft</span>';
                      else if (t && s <= m)
                        lage = '<span class="tag warn">Knapp</span>';
                      else if (t)
                        lage = '<span class="tag ok">In Ordnung</span>';
                      return `<tr>
                        <td class="strong">${esc(p.name)}</td>
                        <td class="muted">${esc(p.category)}</td>
                        <td class="num ${t && s <= m ? "is-low" : ""}">${t ? s : "—"}</td>
                        <td class="num muted">${t ? m : "—"}</td>
                        <td>${lage}</td>
                        <td><div class="row" style="gap:var(--space-2);flex-wrap:nowrap">
                          <button class="btn btn-sm" data-stock-book="${p.id}"${t ? "" : " disabled"}>Buchen</button>
                        </div></td></tr>`;
                    })
                    .join("")
                : `<tr><td colspan="6" class="muted" style="padding:var(--space-8);text-align:center">
        ...
        <div>
          <div class="section-title">Letzte Lagerbewegungen</div>
          <div class="card"><div class="table-wrap"><table class="data">
            <thead><tr><th>Zeit</th><th>Artikel</th><th class="num">Menge</th>
              <th>Grund</th><th>Wer</th></tr></thead>
            <tbody>${
              this.moves.length
                ? this.moves
                    .map(
                      (m) => `<tr>
                        <td class="muted">${fmtDateTime(m.created_at)}</td>
                        <td>${esc(m.product_name)}</td>
                        <td class="num ${Number(m.delta) < 0 ? "is-minus" : "is-plus"}">${
                          Number(m.delta) > 0 ? "+" : ""
                        }${Number(m.delta)}</td>
                        <td><span class="tag">${esc(reasonLabel[m.reason] || m.reason)}</span></td>
                        <td class="muted">${esc(m.staff_name || "—")}</td></tr>`,
                    )
                    .join("")
                : `<tr><td colspan="5" class="muted" style="padding:var(--space-8);text-align:center">
                     Noch keine Bewegungen erfasst.</td></tr>`
            }</tbody></table></div></div>
        </div>
      </div>`;
  },
  ...
  stockForm(p) {
    openModal({
      ...
      bodyHTML: `
        <div class="pay-total">
          <div class="pay-total-label">Aktueller Bestand</div>
          <div class="pay-total-value">${Number(p.stock ?? 0)}</div>
        </div>
        <div class="field">
          <label for="st-reason">Grund</label>
          <select class="select" id="st-reason">
            <option value="wareneingang">Wareneingang — Bestand erhöhen</option>
            <option value="schwund">Schwund oder Bruch — Bestand senken</option>
            <option value="korrektur">Korrektur nach Zählung</option>
          </select>
        </div>
        <div class="field">
          <label for="st-qty">Menge</label>
          <input class="input" id="st-qty" type="number" step="1" min="0" value="10"
                 inputmode="numeric">
        </div>
        ...
        <p class="muted" style="font-size:var(--text-sm);margin-top:var(--space-3)" id="st-preview"></p>`,
      footHTML: `<button class="btn" data-close>Abbrechen</button>
                 <button class="btn btn-primary" id="st-save">Buchen</button>`,
      onMount: (root) => {
        ...
        const delta = () => {
          ...
          if (reason.value === "korrektur")
            return num(v - Number(p.stock ?? 0));
          return reason.value === "wareneingang" ? v : -v;
        };
        const update = () => {
          const d = delta();
          const neu = num(Number(p.stock ?? 0) + d);
          preview.textContent =
            reason.value === "korrektur"
              ? `Neuer Bestand: ${num(Math.abs(qty.value))} Stück (Differenz ${d > 0 ? "+" : ""}${d})`
              : `Neuer Bestand: ${neu} Stück (${d > 0 ? "+" : ""}${d})`;
          $("#st-save", root).disabled = d === 0;
        };
        qty.addEventListener("input", update);
        reason.addEventListener("change", update);
        $("#st-quick", root).addEventListener("click", (e) => {
        ...
        $("#st-save", root).addEventListener("click", async () => {
          const d = delta();
          if (d === 0) return;
          try {
            ...
            closeModal();
            toast(`${p.name}: Bestand gebucht (${d > 0 ? "+" : ""}${d})`);
            await this.loadTab();
            await this.refreshKasse();
          } catch (err) {
            fail(err);
  ...
  renderShifts() {
    ...
    this.shifts.forEach((s) => {
      const key = s.staff_id || s.staff_name;
      if (!map.has(key))
        map.set(key, {
          name: s.staff_name,
          seconds: 0,
      ...
      if (!s.ended_at) e.open = true;
      if (!e.last || new Date(s.started_at) > new Date(e.last))
        e.last = s.started_at;
    });
    const summary = [...map.values()].sort((a, b) => b.seconds - a.seconds);
    const gesamt = summary.reduce((a, e) => a + e.seconds, 0);
    const offen = this.shifts.filter((s) => !s.ended_at);
    ...
    $("#admin-body").innerHTML = `
      <div class="stack">
        <div class="row">
          <div class="subnav" id="sh-range">
            ${ranges
              .map(
                (r) =>
                  `<button data-days="${r.d}" aria-current="${r.d === this.shiftRange}">${r.label}</button>`,
              )
              .join("")}
          </div>
          <span class="spacer"></span>
          <span class="muted" style="font-size:var(--text-sm)">
            Gesamte Dienstzeit: <strong>${fmtDuration(gesamt)}</strong></span>
        </div>
          ...
          offen.length
            ? `<div class="notice">
                 ...
                 ${offen
                   .map(
                     (s) =>
                       `${esc(s.staff_name)} (seit ${fmtTime(s.started_at)}, ${fmtDuration(
                         secs(s) / 1000,
                       )})`,
                   )
                   .join(" · ")}
        ...
        <div>
          <div class="section-title">Dienstzeit je Mitarbeiter</div>
          <div class="card"><div class="table-wrap"><table class="data">
            <thead><tr><th>Mitarbeiter</th><th class="num">Schichten</th>
              <th class="num">Dienstzeit</th><th>Anteil</th><th>Zuletzt</th></tr></thead>
            <tbody>${
              summary.length
                ? summary
                    .map((e) => {
                      const anteil =
                        gesamt > 0 ? Math.round((e.seconds / gesamt) * 100) : 0;
                      return `<tr>
                        <td class="strong">${esc(e.name)}${
                          e.open ? ' <span class="tag ok">im Dienst</span>' : ""
                        }</td>
                        <td class="num">${e.count}</td>
                        <td class="num strong">${fmtDuration(e.seconds)}</td>
                        <td><div class="bar-track" title="${anteil} %">
                          <div class="bar-fill" style="width:${anteil}%"></div></div></td>
                        <td class="muted">${e.last ? fmtDateTime(e.last) : "—"}</td></tr>`;
                    })
                    .join("")
                : `<tr><td colspan="5" class="muted" style="padding:var(--space-8);text-align:center">
                     In diesem Zeitraum war niemand eingestempelt.</td></tr>`
            }</tbody></table></div></div>
        </div>

        <div>
          <div class="section-title">Alle Schichten</div>
          <div class="card"><div class="table-wrap"><table class="data">
            <thead><tr><th>Mitarbeiter</th><th>Beginn</th><th>Ende</th>
              <th class="num">Dauer</th><th>Hinweis</th></tr></thead>
            <tbody>${
              this.shifts.length
                ? this.shifts
                    .map((s) => {
                      const dauer = secs(s) / 1000;
                      const lang = dauer > 12 * 3600;
                      return `<tr>
                        <td class="strong">${esc(s.staff_name)}</td>
                        <td class="muted">${fmtDateTime(s.started_at)}</td>
                        <td class="muted">${
                          s.ended_at
                            ? fmtDateTime(s.ended_at)
                            : '<span class="tag ok">läuft</span>'
                        }</td>
                        <td class="num">${fmtDuration(dauer)}</td>
                        <td>${
                          lang
                    ...
                    .join("")
                : `<tr><td colspan="5" class="muted" style="padding:var(--space-8);text-align:center">
                     Keine Schichten in diesem Zeitraum.</td></tr>`
            }</tbody></table></div></div>
        </div>
      </div>`;

    $("#sh-range").addEventListener("click", (e) => {
      const b = e.target.closest("button[data-days]");
      if (!b) return;
      this.shiftRange = Number(b.dataset.days);
      this.loadTab();
    });
  },
      ...
      onMount: (root) => {
        $("#s-save", root).addEventListener("click", async () => {
          ...
          if (!payload.name)
            return toast("Bitte einen Namen eingeben", "error");
          if (!/^\d{4}$/.test(pin))
            return toast("Der PIN muss aus 4 Ziffern bestehen", "error");
          try {
  ...
  renderClosing(orders) {
    ...
    const discounts = num(
      valid.reduce((s, o) => s + Number(o.discount_amount), 0),
    );
    ...
    valid.forEach((o) =>
      (o.items || []).forEach((i) => {
        ...
        perProduct[i.name].total = num(
          perProduct[i.name].total + Number(i.line_total),
        );
      }),
    );
    ...
    $("#admin-body").innerHTML = `
      <div class="stack">
        ...
        <div>
          <div class="section-title">Umsatz je Mitarbeiter</div>
          <div class="card"><div class="table-wrap"><table class="data">
            <thead><tr><th>Mitarbeiter</th><th class="num">Bestellungen</th>
              <th class="num">Bar</th><th class="num">Karte</th>
              <th class="num">Umsatz</th><th>Anteil</th></tr></thead>
            <tbody>${
              staffRows.length
                ? staffRows
                    .map((e) => {
                      const anteil =
                        revenue > 0 ? Math.round((e.total / revenue) * 100) : 0;
                      return `<tr>
                        ...
                        <td><div class="bar-track" title="${anteil} %">
                          <div class="bar-fill" style="width:${anteil}%"></div></div></td></tr>`;
                    })
                    .join("")
                : `<tr><td colspan="6" class="muted" style="padding:var(--space-8);text-align:center">
                     Heute hat noch niemand verkauft.</td></tr>`
            }</tbody></table></div></div>
        </div>
        <div>
  ...
  async refreshKasse() {
    ...
    State.coops = await DB.listCoops(true);
    if (State.coop && !State.coops.some((c) => c.id === State.coop.id))
      State.coop = null;
    if (
      State.discountId &&
      !State.discounts.some((d) => d.id === State.discountId)
    )
      State.discountId = "";
    State.cart = State.cart.filter((l) =>
      State.products.some((p) => p.id === l.product_id),
    );
    Kasse.render();
  },
  ...
  bind() {
    ...
    $("#admin-body").addEventListener("click", async (e) => {
      ...
      if (t.closest("[data-new-coop]")) return this.coopForm();

      const ec = t.closest("[data-edit-coop]");
      if (ec)
        return this.coopForm(
          this.coops.find((c) => c.id === ec.dataset.editCoop),
        );

      const sb = t.closest("[data-stock-book]");
      if (sb) {
        const p = this.products.find((x) => x.id === sb.dataset.stockBook);
        if (p) this.stockForm(p);
        return;
      }

      const dc = t.closest("[data-del-coop]");
      if (dc) {
        const c = this.coops.find((x) => x.id === dc.dataset.delCoop);
        if (
          c &&
          (await confirmDialog(
            "Kooperation löschen?",
      ...
      if (ep)
        return this.productForm(
          this.products.find((p) => p.id === ep.dataset.editProduct),
        );
      const ed = t.closest("[data-edit-discount]");
      if (ed)
        return this.discountForm(
          this.discounts.find((d) => d.id === ed.dataset.editDiscount),
        );
      const es = t.closest("[data-edit-staff]");
      if (es)
        return this.staffForm(
          this.staff.find((s) => s.id === es.dataset.editStaff),
        );
      ...
      if (ds) {
        ...
        if (!s) return;
        if (s.id === State.user?.id)
          return toast("Das eigene Konto kann nicht gelöscht werden", "error");
        if (
          await confirmDialog(
            "Mitarbeiter löschen?",
